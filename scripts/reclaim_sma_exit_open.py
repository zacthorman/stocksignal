...
def main() -> None:
    ...
    CONFIRM_ON = "open"  # page 116: "FIRST CANDLESTICK HOLDING (OPENING) ABOVE"
    trig = o if CONFIRM_ON == "open" else c
    holds_mask = (trig > fast) & (c > slow) & (fast > slow)
        ...
        for i in range(n_tickers):
            ...
            for t in range(1, n_days):
                ...
                for u in range(entry_bar, limit + 1):
                    if np.isfinite(trig_i[u]) and np.isfinite(fast_i[u]) and trig_i[u] < fast_i[u]:
                        streak += 1
    ...
    print(f"entry: RSI<30 arms ({ARM_WINDOW} sessions), candle OPENS above the 9 SMA "
          f"(page 116) and closes above the 180 SMA, next-open fill")
    print("exit:  first candle OPENING below the 9 SMA (page 111), next-open fill")
